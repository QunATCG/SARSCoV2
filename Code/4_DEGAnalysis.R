...
# get normalized read counts for PCA plotting
{
  ...
  data_PCA <- as.data.frame(t(exp_data_readcounts[exp_data_readcounts$Variance != 0,][,1:9]))
  
  data_PCA_prcomp <- prcomp(data_PCA, scale. = T)
  
  # calculate percentage of PCs
  data_PCA_var <- data_PCA_prcomp$sdev^2
  data_PCA_per <- round(data_PCA_var/sum(data_PCA_var)*100,1)
  write.table(data_PCA_per, "./Results/Table/exp_Data_PCA_percentage.txt", quote = F, sep = "\t", row.names = T)
  # PCs 
  data_PCA_output <- data.frame(data_PCA_prcomp$x)
  write.table(data_PCA_output, "./Results/Table/exp_Data_PCA.txt", quote = F, sep = "\t", row.names = T)
  
  # correlation of samples
  data_sample_cor <- cor(exp_data_readcounts[,1:9])
  write.table(data_sample_cor, "./Results/Table/exp_Data_Correlation.txt", quote = F, sep = "\t", row.names = T)
}
...
# DE analysis
{
  ...
  ## RSEM-limma
  # https://pubmed.ncbi.nlm.nih.gov/38049604/
